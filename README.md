...
| `@mdy-docs/nisaba-db/server-client` | WASM-free client for a database *server* process (`server/main.c`) over a socket — `connectServer('host:port')`, twenty-three operations — CRUD, lists of writes, aggregation, paged cursors, indexes, compaction, listing — no engine in the client |
