...
| `@mdy-docs/nisaba-db/server-client` | WASM-free client for a database *server* process (`server/main.c`) over a socket — `connectServer('host:port')`, twenty-nine operations — everything the in-process `Collection` has except `watch()` — no engine in the client |
