...
| `@mdy-docs/nisaba-db/server-client` | WASM-free client for a database *server* process (`server/main.c`) over a socket — `connectServer('host:port')`, nineteen operations — CRUD, paged cursors, indexes, compaction — no engine in the client |
