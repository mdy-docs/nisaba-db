 ...
 * Run after `npm run build` (package.json's `test`). The Node provider
 * is exercised too, against a temp directory, because `./node` is the
 * one entry with platform-specific imports.
 */
...
const { connect, MemoryStorageProvider } = await import('./src/db.js');
...
const { connect: connectViaNode, NodeFSStorageProvider } = await import('./src/db-node.js');
